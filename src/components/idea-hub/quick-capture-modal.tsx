...
import { FilePen, X, Calendar, Tag, ImagePlus, Trash2, Sparkles, List, Link2 } from "lucide-react";
import { useAppStore, useIdeaStore, useWorkspaceStore } from "@/stores";
import type { CheckListItem } from "@/stores";
import { IdeaChecklist } from "./idea-checklist";

/* ------------------------------------------------------------------ */
/*  Local image type (stored until idea is created)                     */
/* ------------------------------------------------------------------ */
...
export function QuickCaptureModal() {
  ...
  const isOpen = useAppStore((s) => s.isQuickCaptureOpen);
  ...
  const [localImages, setLocalImages] = useState<ReadonlyArray<LocalImage>>([]);
  ...
  const processImageFile = useCallback((file: File) => {
    ...
    reader.onload = () => {
      ...
      setLocalImages((prev) => [
        ...prev,
        { id: crypto.randomUUID(), data: Array.from(uint8), ext, preview },
      ]);
    };
  ...
  // Global paste listener for the modal
  useEffect(() => {
    if (!isOpen) return;
    document.addEventListener("paste", handlePasteImage);
    return () => document.removeEventListener("paste", handlePasteImage);
  }, [isOpen, handlePasteImage]);

  const handleRemoveLocalImage = useCallback((id: string) => {
    setLocalImages((prev) => {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      if (plannedDate) {
        await updatePlannedDate(idea.id, plannedDate);
      }
      if (checklist.length > 0) {
        await updateChecklist(idea.id, checklist);
      }
      // Save images after idea is created
      if (localImages.length > 0) {
        const filenames: string[] = [];
        for (const img of localImages) {
          const filename = await saveImage(idea.id, img.data, img.ext);
          filenames.push(filename);
        }
        await updateImages(idea.id, filenames);
      }
      handleClose();
    } finally {
      setIsSaving(false);
          ...
          <div className="flex flex-col" style={{ gap: 16 }}>
            ...
            <div className="flex flex-col" style={{ gap: 8 }}>
                ...
                style={
                  {
                    ...
                    "--native-hover-border-color": "var(--color-text-tertiary)",
                  } as React.CSSProperties
                }
              ...
              >
                <span
                  className="flex items-center"
                  style={{ fontSize: 10, color: "var(--color-text-tertiary)", gap: 4 }}
                >
                  <ImagePlus size={14} style={{ color: "var(--color-text-tertiary)" }} />{" "}
                  {t("ideaHub.create.imageDrop")}
                </span>
              </div>
            </div>
